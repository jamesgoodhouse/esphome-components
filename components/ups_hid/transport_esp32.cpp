...
esp_err_t Esp32UsbTransport::teardown_usb_host() {
    ...
    // under the mutex to avoid double-release/close.
    {
        ...
        if (dev && cli) {
            ...
            // Per ESP-IDF docs: halt and flush all claimed endpoints before
            // releasing the interface.  This ensures endpoint pipes are in a
            // clean state so interface_release and device_close don't crash.
            uint8_t eps[] = {device_.ep_in, device_.ep_out};
            for (auto ep : eps) {
                if (ep == 0) continue;
                esp_err_t hr = usb_host_endpoint_halt(dev, ep);
                ESP_LOGD(ESP32_USB_TAG, "  endpoint_halt(0x%02X): %s", ep, esp_err_to_name(hr));
                if (hr == ESP_OK || hr == ESP_ERR_INVALID_STATE) {
                    esp_err_t fr = usb_host_endpoint_flush(dev, ep);
                    ESP_LOGD(ESP32_USB_TAG, "  endpoint_flush(0x%02X): %s", ep, esp_err_to_name(fr));
                }
            }

            // Drain any transfer callbacks generated by halt/flush.
            for (int i = 0; i < 10; i++) {
                esp_err_t ev_ret = usb_host_client_handle_events(cli, pdMS_TO_TICKS(50));
                if (ev_ret == ESP_ERR_TIMEOUT) break;
            }
            ...
            ESP_LOGI(ESP32_USB_TAG, "  interface_release: %s", esp_err_to_name(rel_ret));
            ...
            ESP_LOGI(ESP32_USB_TAG, "  device_close: %s", esp_err_to_name(close_ret));
        }
...
void Esp32UsbTransport::process_device_gone_locked() {
    ...
    if (device_.dev_hdl && device_.client_hdl) {
        ...
        for (auto ep : eps) {
            if (ep == 0) continue;
            esp_err_t hr = usb_host_endpoint_halt(device_.dev_hdl, ep);
            ESP_LOGD(ESP32_USB_TAG, "  endpoint_halt(0x%02X): %s", ep, esp_err_to_name(hr));
            if (hr == ESP_OK || hr == ESP_ERR_INVALID_STATE) {
                esp_err_t fr = usb_host_endpoint_flush(device_.dev_hdl, ep);
                ESP_LOGD(ESP32_USB_TAG, "  endpoint_flush(0x%02X): %s", ep, esp_err_to_name(fr));
            }
        }

        // Drain callbacks generated by halt/flush.
        for (int i = 0; i < 10; i++) {
            esp_err_t ev = usb_host_client_handle_events(device_.client_hdl, pdMS_TO_TICKS(50));
            if (ev == ESP_ERR_TIMEOUT) break;
        }

        esp_err_t rel_ret = usb_host_interface_release(device_.client_hdl, device_.dev_hdl, device_.interface_num);
